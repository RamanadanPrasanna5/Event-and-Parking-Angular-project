...
import { CreateVenueDto, Venue } from '../../../core/models/venue.model';
            ...
            <table class="custom-table">
              ...
              <tbody>
                @for (v of venues(); track v.id) {
                  <tr>
                    <td><strong class="text-main">{{ v.name }}</strong></td>
                    <td>{{ v.location }}</td>
                ...
                <div class="d-flex justify-content-end gap-2">
                  ...
                  <button type="submit" class="btn btn-primary" [disabled]="venueForm.invalid || saving()">
                    @if (saving()) {
                      ...
                      <i class="fa-solid fa-floppy-disk me-1"></i> Save Venue
                    }
                  </button>
                </div>
              </form>
  ...
  confirmDelete(): void {
    ...
    this.venueService.deleteVenue(this.selectedVenue.id).subscribe({
      next: () => {
        ...
        this.toastService.success('Venue deleted successfully.');
        this.loadVenues();
      },
      error: (err) => {
        this.deleting.set(false);
